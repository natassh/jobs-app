import { getJobsPagedService } from '../getJobsPagedService';
import * as getJobListPagedModule from "../../../../../../core/services/jobs/getJobListPaged";

describe('getJobsPagedService', () => {

  it('return an array with the jobs', async () => {
    // Arrenge 
    const filters = {
      description: "",
      location: "",
    ...
    const getJobListPagedMock = jest.spyOn(getJobListPagedModule, 'getJobListPaged');  // spy on otherFn
    getJobListPagedMock.mockReturnValue(mockReturnNewJobs); 


    // Act
    const newJobs = await getJobsPagedService(filters, pageNumber);

    // Assert
    expect(newJobs).toHaveLength(50)
  })
})